...
void LoginRoom()     //登记客房信息函数
{
    ...
    scanf("%s",(char *)&Room[i-1].Clientlist.Sex);
...
void save()      //将客房入住信息保存到文件中
{
    //int i;
    FILE *fp;
    if((fp=fopen("D:\\Desktop\\aa.txt","w"))==NULL)
    {
        printf("无法打开文件\n");
        return;
    }
...
void load()      //从文件中读取客房入住信息
{
    FILE *fp;
    int i;
    if((fp=fopen("D:\\Desktop\\aa.txt","r"))==NULL)
    {
        printf("不能打开文件\n");
        return;
    }
...
void ChangeRoom()      //修改客房入住信息
{
    ...
    if(Room[i-1].Sign==1)
    {
        ...
        switch(n)
        {
            ...
            break;
            case 2:scanf("%s",(char *)&Room[i-1].Clientlist.Sex);
            break;
